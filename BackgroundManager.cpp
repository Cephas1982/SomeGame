...
void C_BackgroundManager::RenderLayer1()
{
	for(int i = 0; i < v_bgObjectsActive.size(); i++){
		if(v_bgObjectsActive[i].Get_layer() == 1){//don't do anything if object is not on layer 3
			//correct for camera
...
{
	return v_bgObjectsActive[element].Get_layer();
}
...
void C_BackgroundManager::Save()
{
	...
	//SAVE ACTIVE BG INFO
	for(int i = 0; i < v_bgObjectsActive.size(); i++){
		//Save which type of obj (sprite sheet clip)
		...
		if(i == v_bgObjectsActive.size() -1)//if on the very last object
			outFile << v_bgObjectsActive[i].Get_Position().y;//do not use endl;
		else
			outFile << v_bgObjectsActive[i].Get_Position().y << std::endl;
	}
